...
TEST(MicMixerTest, SkippingHostFramesExpiresOverflowRecoveryWindow) {
  ...
  // Sequence 251 maps to the first slot after re-buffering two frames on the new playout clock.
  // Skipping host frames must expire the old overflow window instead of re-anchoring.
