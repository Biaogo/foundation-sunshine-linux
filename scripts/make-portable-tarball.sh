...
#
# 本脚本属于分发仓库（fork）。构建产物的 Nix 闭包来自
# Biaogo/foundation-sunshine-linux.nix（NixOS flake 仓库）。
#
# Why: the flake's release tarballs are raw Nix store outputs — on a host
# without Nix they fail ("bad interpreter", "required file not found",
# every lib "not found"): the ELF interpreter, RPATHs and wrapper shebangs
# are absolute /nix/store paths. The closure ITSELF must ship.
#
...
# Result: a self-contained bundle with its own glibc/ld.so; the target
# machine needs NOTHING (no Nix, no patchelf, no fuse — works on older
# distros since glibc 2.42 rides along). TLS uses the host CA bundle.
#
# Install on the target:
#   sudo tar -C / -xzf foundation-sunshine-<version>-portable-linux-x86_64.tar.gz
#   /opt/sunshine-portable/bin/sunshine
# See docs/portable-tarball.md for the full user/maintainer guide.
#
# Usage (BUILD host needs nix + patchelf; the TARGET needs nothing):
#   scripts/make-portable-tarball.sh [OUT.tar.gz] [--store-path <path>] [--prefix <p>]
#   --store-path: a foundation-sunshine store path (e.g. from cachix). Without
#     it the CPU variant is built via the flake repo
#     (github:Biaogo/foundation-sunshine-linux.nix#foundation-sunshine-upstream).
#   --prefix:     target install prefix (default /opt/sunshine-portable)
set -euo pipefail
...
if [ -z "$SRC" ]; then
  echo "== no --store-path: building CPU variant via github:Biaogo/foundation-sunshine-linux.nix"
  SRC=$(nix build github:Biaogo/foundation-sunshine-linux.nix#foundation-sunshine-upstream \
