...
**配置的路径：目录在就照用（2026-09-26 补）**：可执行性检查只是防手滑，它在这台机器上会**误报** ——
同一个 helper 路径可能出现"`access()` 说不在、`stat`/`ls` 说在"的分钟级窗口（运行中的服务也撞上过）。
那种窗口里旧的逻辑会把整条虚拟屏判定为"不可用"：**客户端列表里两个虚拟选项直接消失**，会话也会落到
枚举到的第一块物理屏（eDP-1）上。现在的规则更精确：
- 配置路径的**父目录存在**（叶子读不到）→ 视为误报，**照用**，只打一条
  `could not be verified as an executable file right now; using it anyway`；
- 配置路径的**父目录也不存在** → 当作写错，继续按 `$PATH` / 标准目录搜（保持防手滑能力）。

真写错到能启动的那一步时，会在**启动 helper 时**响亮失败，而不是静默把功能藏起来。`$PATH` 搜索照旧做检查（15/60/200 ms 重试）。
